...
| Historical `.kiro` specs (`ef28ff6`) | Requirements documented photo avatars, 20 panels, and selective regeneration but also stale architecture/security assumptions. | Deleted the three specs as obsolete planning artifacts. | Reduce stale/misleading public documentation. | Exact specs **Keep deleted**; intent **Restore and implement** in a current contract. | Create one authoritative product/mode contract linked to this audit. | Same full contract. | Demo limitations documented separately. | Founder must decide story length; stale specs must not silently govern runtime. |
...
| Strict provider-output contracts (dirty Task 14) | Story ideas/scripts used permissive JSON, padding/default normalization, oversized prompt context, and review-on defaults. | Uncommitted Pydantic contracts require exactly 3 ideas and sequential bounded panels/speakers; services use structured parsing/token caps; narrative context drops IDs/avatar URLs; review defaults off and attempts are capped. | Reject invalid model output before persistence/provider fan-out and minimize personal context/cost. | **Keep direction**, preserve the seven-file dirty set; adapt after story-length decision. | Strict contracts plus fictional prompt evals; no persistence until validation. | Reuse the same response schema with fixtures. | Currently hard-codes 8–12, still deletes panels before replacement succeeds, and has no durable worker. |
...
| Student/account deletion and leave | Database had simple row deletes and an unauthenticated leave route; UI Delete Account only logged/navigated. No complete file cleanup contract exists. | Removed fake Delete Account; leave API remains unused and public. | Avoid false deletion, but the destructive boundary remains incomplete. | Fake UI **Keep deleted**; **Restore and implement**. | Private release separates leave-classroom, delete student profile, delete chapter, and reset-local-data operations; it removes recorded local files and rows with idempotent retry. Hosted account/session deletion comes later. | No durable delete claim; reset session-local persona state only. | Shared stories may contain a likeness or name. The founder must choose their erasure behavior. Hosted Supabase user deletion requires object cleanup first. |
...
1. **Story length: fixed 20 panels, 8–12, or selectable?**  
   Recommendation: make 12 the cost/latency-conscious default and offer an explicit 20-panel `Full comic` option in private and hosted settings. Resolve this before committing the dirty Task 14 contract.

2. **What should full deletion do to historical stories containing a student's name or likeness?**  
   Recommendation: distinguish leaving a classroom from erasure. Erasure should delete the student's source photo, avatar, profile, and provider-input records, then delete affected story assets or require teacher-confirmed regeneration/anonymization. Do not claim complete deletion while retaining identifiable panels.

3. **Photo retention after successful avatar creation?**  
...
4. **Material scope: native-text PDFs only or OCR/scanned documents too?**  
...
5. **Public-demo state model: fully read-only or session-local edits/simulation?**  
...
6. **Real-minor provider boundary?**  
...
7. **Public repository history and license?**  
